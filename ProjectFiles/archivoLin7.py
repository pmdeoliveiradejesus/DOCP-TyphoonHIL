...
def funAr7(model, hcpd, datos, tipoCurva, nFallas, tiempoFalla, tasaMuestreo, durSim):
    #Constantes de relé
    ...
    for i in range(len(l1)):
        ...
        #CPCS
        Line1 = mdl.get_item("Line-7_section-1", item_type=const.ITEM_MASKED_COMPONENT)
        Line2 = mdl.get_item("Line-7_section-2", item_type=const.ITEM_MASKED_COMPONENT)
        ...
        while hil.get_sim_time()<tiempoFalla:
            esperar=1
        start_capture(duration=durSim, rate=tasaMuestreo, signals=["Relay-7.Function (67P).I_A(RMS)","Relay-14.Function (67P).I_A(RMS)","Relay-5.Function (67P).I_A(RMS)","Relay-13.Function (67P).I_A(RMS)","Relay-9.Function (67P).I_A(RMS)","Relay-1.Function (67P).I_A(RMS)"])
        ...
        index=capture.index
        I7_data=np.multiply(dataCap[:,1],1)
        I14_data=np.multiply(dataCap[:,2],1)
        I5_data=np.multiply(dataCap[:,3],1)
        I13_data=np.multiply(dataCap[:,4],1)
        I9_data=np.multiply(dataCap[:,5],1)
        I1_data=np.multiply(dataCap[:,6],1)
        ...
        if releQ==7:
            ...
            releJ1=9
            releJ2=1
            Relay1 = [releQ,releP1,releI,releJ1,releI,releJ1]
            Relay2 = [releQ,releP2,releI,releJ2,releI,releJ2]
            OTIpre=OT14
            OTP1=OT5
            OTP2=OT13
            OTJ1pre=OT9
        ...
        #for i in range(len(Ii_data)):
            #Iip.append(Ii_data[i]-II_Data[i])
            #Iip.append(simTimeT[i])
            #Ir1p.append(I14_data[i])
            #Ir1p.append(simTime[i])
        #print(Iip)
        #print(Ir1p)
        #print(Ir2p)
        ...
        #Identificacion de relés primarios y secundarios para transientes CPCS
        tiemposOperacion1 = [OTQ,OTP1,OTP2,OTi,OTRe1,OTRe2]
        relesExp1 = [releQ, releP1, releP2, releI, releJ1, releJ2]
